...
impute_before_spearman_array <- c()
impute_after_spearman_array <- c()
for(j in 1 : 30) {
  OTU <- read.csv("PRJEB7774.csv", row.names="X")
  OTU <- OTU / colSums(OTU) * 10^6 
  OTU <- log10(OTU+1)
  OTU_ori_bool <- OTU != 0
  OTU_downsample <- read.csv(paste0("PRJEB7774_remove_", remove_rate * 100, "_", j, ".csv"), row.names="X")
  OTU_impute <- read.csv(paste0("PRJEB7774_remove_", remove_rate * 100,"_impute_normalize_", j, ".csv"), row.names="X")
  ...
  downsample_spearman_coefficient_mean <- 0
  for (i in 1:dim(OTU)[1]) {
    downsample_spearman_coefficient_mean <- downsample_spearman_coefficient_mean + cor(OTU[i, ], OTU_downsample[i, ], method="spearman")
  }
  downsample_spearman_coefficient_mean <- downsample_spearman_coefficient_mean / dim(OTU)[1]
  impute_before_spearman_array <- c(impute_before_spearman_array, downsample_spearman_coefficient_mean)
  cat("impute_before_spearman:", downsample_spearman_coefficient_mean, '\n')
  
  OTU_ori_impute <- t(read.csv("PRJEB7774_impute_normalize.csv", row.names="X"))
  impute_spearman_coefficient_mean <- 0
  for (i in 1:dim(OTU_ori_impute)[1]) {
    impute_spearman_coefficient_mean <- impute_spearman_coefficient_mean + cor(OTU_ori_impute[i, ], OTU_impute[i, ], method="spearman")
  }
  impute_spearman_coefficient_mean <- impute_spearman_coefficient_mean / dim(OTU_ori_impute)[1]
  impute_after_spearman_array <- c(impute_after_spearman_array, impute_spearman_coefficient_mean)
  cat("impute_after_spearman:", impute_spearman_coefficient_mean, '\n')
}
...
cat("impute_before_spearman_mean:", mean(impute_before_spearman_array), '\n')
cat("impute_before_spearman_sd:", sd(impute_before_spearman_array), '\n')
cat("impute_after_spearman_mean:", mean(impute_after_spearman_array), '\n')
cat("impute_after_spearman_sd:", sd(impute_after_spearman_array), '\n')
