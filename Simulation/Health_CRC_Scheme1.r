...
CRC_real_data <- read.table("CRC.txt", header = TRUE, sep = "\t", row.names=1)
CRC_real_data[is.na(CRC_real_data)] <- 0

union_rows <- union(row.names(control_real_data), row.names(CRC_real_data))
data <- CRC_real_data
...
CRC_real_data <- t(CRC_real_data)
real_data <- rbind(control_real_data, CRC_real_data)
...
meta_sim <- matrix(0, nrow = 35, ncol = 4)
...
write.csv(sim_tab_zi_trans, "Health_CRC_zero_1.csv")
simulated2_trans <-t(10^(simulated2) - 1.01)
write.csv(simulated2_trans, "Health_CRC_full_1.csv")
...
write.csv(meta_real_data, "Health_CRC_Metadata.csv")
...
data_full_eval <- data_full_eval / scale
data_full_eval <- log10(data_full_eval + 1)
GEImpute <- t(read.csv("geimpute_Health_CRC_impute_1.csv", row.names = "X"))
...
data_full_eval <- data_full_eval / scale
data_full_eval <- log10(data_full_eval + 1)
mbSparseImpute <- t(read.csv("Health_CRC_impute_normalize_1.csv", row.names = "X"))
size <- dim(data_full_eval)[1] * dim(data_full_eval)[2] 
