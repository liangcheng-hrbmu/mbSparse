...
write.csv(sim_tab_zi_trans, "Health_CRC_zero_2.csv")
simulated2_trans <-t(10^(simulated2) - 1.01)
write.csv(simulated2_trans, "Health_CRC_full_2.csv")
...
data_full_eval <- data_full_eval / scale
data_full_eval <- log10(data_full_eval + 1)
GEImpute <- t(read.csv("geimpute_Health_CRC_impute_2.csv", row.names = "X"))
...
data_full_eval <- data_full_eval / scale
data_full_eval <- log10(data_full_eval + 1)
mbSparseImpute <- t(read.csv("Health_CRC_impute_normalize_2.csv", row.names = "X"))
size <- dim(data_full_eval)[1] * dim(data_full_eval)[2] 
