...
def imputation(scfile, k, feature_train_epochs, feature_pretrain_epochs, cave_train_epochs, unnormalized=True, samples_are_rows=False):  
    np_scfile = np.array(scfile)
    if samples_are_rows == True:
        np_scfile = np_scfile.T
    ...
    if samples_are_rows == True:
        imputation_file = imputation_file.T
